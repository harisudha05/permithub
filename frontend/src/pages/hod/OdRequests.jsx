...
import { sendOdWhatsApp } from '../../utils/whatsapp'

function StatusBadge({ status }) {
  const c = {
    PENDING:'bg-yellow-100 text-yellow-800', HOD_APPROVED:'bg-green-100 text-green-800',
    ADVISOR_APPROVED:'bg-blue-100 text-blue-700', COORDINATOR_APPROVED:'bg-purple-100 text-purple-700',
    MENTOR_APPROVED:'bg-blue-100 text-blue-600', REJECTED:'bg-red-100 text-red-700',
  }
  return (
...
export default function HodOd() {
  ...
  const load = async () => {
    ...
    try {
      const res = await odApi.pendingHod()
      const data = Array.isArray(res.data) ? res.data
                 : Array.isArray(res.data?.data) ? res.data.data : []
      setItems(data)
    } catch (e) {
      setError(e.response?.data?.message || 'Failed to load OD requests')
  ...
  const action = async (id, approved, remarks) => {
    setActionLoading(id)
    try {
      await odApi.hodAction(id, { approved, remarks })
      load()
    } catch (e) {
  ...
  return (
    <div>
      <div className="bg-white border-b border-gray-100 px-6 py-4">
        <div className="text-lg font-semibold text-gray-900">Pending OD Requests</div>
        <div className="text-xs text-gray-400 mt-0.5">
          Awaiting your HOD approval — approved by Mentor, Coordinator &amp; Class Advisor
        </div>
      </div>
      <div className="p-6">
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-12 text-center text-gray-400 text-sm">
            No pending OD requests for your department
          </div>
        ) : (
...
function OdCard({ r, loading, notifying, onApprove, onReject, onNotify }) {
    ...
    <div className="bg-white rounded-xl border border-gray-100 p-5">
      ...
      <div className="flex items-start justify-between mb-3">
        <div>
          ...
          <div className="text-xs text-gray-500 mt-1">{r.departmentName}</div>
        </div>
      ...
      <div className="bg-gray-50 rounded-lg p-3 mb-3">
        <div className="flex items-center gap-2 mb-1 flex-wrap">
          ...
          <span className="text-sm font-medium text-gray-800">{r.eventName}</span>
        </div>
        {r.organizer && <div className="text-xs text-gray-500">Organizer: {r.organizer}</div>}
        {r.location && <div className="text-xs text-gray-500">Location: {r.location}</div>}
        {r.description && <div className="text-xs text-gray-500 mt-1 italic">{r.description}</div>}
      </div>
      ...
      <div className="flex items-center gap-1 mb-4 flex-wrap">
        {[
          ...
          { label: 'Coordinator', status: r.coordinatorStatus, name: r.coordinatorName, remarks: r.coordinatorRemarks },
          { label: 'Advisor',     status: r.advisorStatus,     name: r.advisorName,     remarks: r.advisorRemarks },
          ...
          <div key={step.label} className="flex items-center">
            <div className="flex flex-col items-center">
              ...
              <span className="text-xs mt-0.5 font-medium text-gray-600 whitespace-nowrap">{step.label}</span>
              {step.name && <span className="text-xs text-gray-400 whitespace-nowrap">{step.name}</span>}
              {step.remarks && (
                <span className="text-xs text-gray-400 italic max-w-[80px] truncate text-center" title={step.remarks}>
                  "{step.remarks}"
                </span>
              )}
            </div>
            {i < arr.length - 1 && (
              <div className={`h-0.5 w-6 mx-1 mt-[-20px] ${step.status === 'APPROVED' ? 'bg-green-400' : 'bg-gray-200'}`} />
            )}
          </div>
        ))}
      </div>
      ...
      <div className="border-t border-gray-50 pt-3">
        <p className="text-xs text-blue-600 font-medium mb-2">⏳ Awaiting your final approval as HOD</p>
        <div className="flex items-center gap-2 flex-wrap">
          <input
        ...
        <p className="text-xs text-gray-400 mt-2">
          {r.parentWhatsapp
            ? `📱 ${r.parentWhatsapp}`
            : '⚠️ Parent WhatsApp not in DB — click Notify Parent to enter manually'}
        </p>
      </div>
    </div>
  )
